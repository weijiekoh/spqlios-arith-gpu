...
pub fn create_sb_with_data<T: bytemuck::NoUninit>(
    device: &wgpu::Device,
    data: &[T],
) -> wgpu::Buffer {
...
pub async fn finish_encoder_and_read_from_gpu<T: bytemuck::NoUninit + bytemuck::Pod>(
    device: &wgpu::Device,
    queue: &wgpu::Queue,
    ...
    let mut result: Vec<Vec<T>> = Vec::with_capacity(bytes.len());
    for r in bytes {
...
pub mod tests {
    ...
    pub async fn test_get_device_and_queue() {
        ...
        let poll_result = device.poll(wgpu::PollType::Poll).unwrap();
