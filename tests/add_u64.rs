...
#[tokio::test]
pub async fn test_add_u64() {
    let (device, queue) = get_device_and_queue().await;

    let val = vec![123u64];

    let val_buf = create_sb_with_data(&device, &val);
    let result_buf = create_empty_sb(&device, (std::mem::size_of::<u64>()) as u64);

    let source = render_simple("add_u64.wgsl");
    ...
    let results =
        finish_encoder_and_read_from_gpu::<u64>(&device, &queue, Box::new(command_encoder), &[
            result_buf,
        ])
        .await;
